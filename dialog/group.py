...
class DynamicRowGroup(Group):

    def __init__(self, *buttons: Keyboard, id: Optional[str] = None, width: int = None,
                 when: WhenCondition = None, number_of_characters: int = 14 ):
        super().__init__(id, when)
        self.buttons = buttons
        self.width = width
    ...
    def _wrap_kbd(self, kbd: List[InlineKeyboardButton]) -> List[List[InlineKeyboardButton]]:
        ...
        for b in kbd:
            len_text = len(b.text)
            if len_text <= self.number_of_characters:
                small_text.append(b)
            else:
                res.append([b])
            if len(small_text) >= self.width:
                res.append(small_text)
                small_text = []
        if small_text:
            res.append(small_text)
        return res
